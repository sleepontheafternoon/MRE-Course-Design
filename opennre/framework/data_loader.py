...
class SentenceREDataset(data.Dataset):
    ...
    """
    def __init__(self, text_path, pic_dir, rel2id, tokenizer, kwargs):
        """
        Args:
        ...
        f.close()
        logging.info("Loaded sentence RE dataset {} with {} lines and {} relations.".format(text_path, len(self.data), len(self.rel2id)))
        ...
        for picName in pathList:
            ...
            self.pic_path_dict[picName] = pic_path
    ...
    def __getitem__(self, index):
        ...
        # 分词id 位置掩码 两个实体的pos
        seq = list(self.tokenizer(item, **self.kwargs))  # i 只处理token h tail relation mask
        #   一个分词编码 一个注意力编码 返回两个位置编码
        # pos1 = seq[-1]
        # pos2 = seq[-2]
        image_path = self.pic_path_dict[item['img_id']]
        # image = cv2.imread(image_path)
        # jpeg4py速度比cv2快
        image = jpeg4py.JPEG(image_path).decode()
        # image = Image.open(image_path)
        img_tensor = torch.from_numpy(image).permute((2, 0, 1)) #.unsqueeze(0) # 3 600 800
        res = [self.rel2id[item['relation']]] + seq + [img_tensor]
    ...
    def collate_fn(data):
        ...
        return [batch_labels] + batch_seqs + [list(data[-1])]
...
def SentenceRELoader(text_path, pic_dir, rel2id, tokenizer, batch_size,
                     shuffle, num_workers=8, collate_fn=SentenceREDataset.collate_fn, **kwargs):
    dataset = SentenceREDataset(text_path=text_path, pic_dir=pic_dir,
                                rel2id=rel2id,
                                tokenizer=tokenizer, 
                                kwargs=kwargs)
