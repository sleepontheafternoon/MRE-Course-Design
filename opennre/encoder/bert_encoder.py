...
class BERTEntityEncoder(nn.Module):
    ...
    def forward(self, token, att_mask, pos1, pos2):
        ...
        onehot_head = onehot_head.scatter_(1, pos1, 1) # 64 128  生成头尾的独热编码
        onehot_tail = onehot_tail.scatter_(1, pos2, 1) # 64 128
        head_hidden = (onehot_head.unsqueeze(2) * hidden).sum(1)  # (B, H)  64 768
        tail_hidden = (onehot_tail.unsqueeze(2) * hidden).sum(1)  # (B, H) 64 768
    ...
    def tokenize(self, item):
        ...
        if pos_head[0] > pos_tail[0]:
            ...
            rev = False
        # 实体作为区分 实体之前的句子 head  head，tail 之间的句子 tail tail之后的句子
        if not is_token:
            sent0 = self.tokenizer.tokenize(sentence[:pos_min[0]])
            ent0 = self.tokenizer.tokenize(sentence[pos_min[0]:pos_min[1]])
            sent1 = self.tokenizer.tokenize(sentence[pos_min[1]:pos_max[0]])
        ...
        re_tokens = ['[CLS]'] + sent0 + ent0 + sent1 + ent1 + sent2 + ['[SEP]']  # 句子开头与结束需要特殊符号
        # 定位两个实体unxxx部分位置
        pos1 = 1 + len(sent0) if not rev else 1 + len(sent0 + ent0 + sent1)
        pos2 = 1 + len(sent0 + ent0 + sent1) if not rev else 1 + len(sent0)
        pos1 = min(self.max_length - 1, pos1)
        ...
        indexed_tokens = self.tokenizer.convert_tokens_to_ids(re_tokens)  # 将分词转换成编号
        avai_len = len(indexed_tokens)
        ...
        # Attention mask
        att_mask = torch.zeros(indexed_tokens.size()).long()  # (1, L)
        att_mask[0, :avai_len] = 1  # 前面多少地方有意义
